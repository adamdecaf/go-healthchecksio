...
func TestCheckLifecycle(t *testing.T) {
	...
	created, err := client.CreateCheck(ctx, createReq)
	...
	t.Cleanup(func() {
		_, err := client.DeleteCheck(ctx, created.UUID)
		if err != nil {
	...
	gotByUUID, err := client.GetCheck(ctx, created.UUID)
	require.NoError(t, err)
	...
	listResp, err := client.GetChecks(ctx, healthchecksio.GetChecks{
	...
	updated, err := client.UpdateCheck(ctx, created.UUID, updateReq)
	require.NoError(t, err)
	require.Equal(t, "Updated Name", updated.Name)
	require.Equal(t, 3600, updated.Grace)

	// Pause the check
	paused, err := client.PauseCheck(ctx, created.UUID)
	require.NoError(t, err)
	require.NotNil(t, paused)

	// Resume the check
	resumed, err := client.ResumeCheck(ctx, created.UUID)
	require.NoError(t, err)
	require.NotNil(t, resumed)

	// Send a ping (success)
	err = client.Ping(ctx, created.PingURL, "")
	require.NoError(t, err)
	...
	pings, err := client.GetPings(ctx, created.UUID)
	require.NoError(t, err)
	...
	// Send a failure ping
	err = client.Ping(ctx, created.PingURL, "example body", healthchecksio.WithFail())
	require.NoError(t, err)
	...
	body, err := client.GetPingBody(ctx, created.UUID, 2)
	require.NoError(t, err)
	require.Equal(t, "example body", body)

	// Check flips
	flips, err := client.GetFlips(ctx, created.UUID, healthchecksio.GetFlipsRequest{})
	require.NoError(t, err)
...
func TestCreateWithMinimalFields(t *testing.T) {
	...
	created, err := client.CreateCheck(ctx, &healthchecksio.CreateCheck{
	...
	t.Cleanup(func() {
		client.DeleteCheck(ctx, created.UUID)
	})
}
